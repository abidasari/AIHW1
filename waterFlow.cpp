//////Abhinav Dasari USC ID:4206370716
#include <iostream>
#include <fstream>
#include <stdio.h>
#include <string>
...
int main (int argc, char *argv[])
{
	...
	while(!myfile.eof())
	{
		...
		string::iterator end_pos = remove(Alg.begin(), Alg.end(), ' ');
		Alg.erase(end_pos, Alg.end());
		getline(myfile, src); //cout << "source: " << src << " ." << endl;//source vertex string
		string::iterator end_pos1 = remove(src.begin(), src.end(), ' ');
		src.erase(end_pos1, src.end());
